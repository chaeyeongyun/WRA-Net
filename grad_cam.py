...
class GradCAM():
    def __init__(self, model:torch.nn.Module, target_layers:List[nn.Module], device:torch.device):
        ...
        for target_layer in target_layers:
            self.handles += [target_layer.register_forward_hook(self.save_activation_hook)]
            self.handles += [target_layer.register_forward_hook(self.save_gradient_hook)]
...
if __name__ == "__main__":
    ...
    main(class_num=1, 
        img_dir='/content/data/cropweed_total/CWFID/seg/train/input', 
        model=model, 
